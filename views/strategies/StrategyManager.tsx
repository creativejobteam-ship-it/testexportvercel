...
import { getStrategies as getLocalStrategies, deleteStrategy as deleteLocalStrategy, addNewStrategy } from '../../services/strategyService';
import { getStrategies as getLiveStrategies, deleteStrategy as deleteLiveStrategy } from '../../src/services/dbService';
...
const StrategyManager: React.FC<StrategyManagerProps> = ({ onNavigate }) => {
  ...
  const refreshStrategies = async () => {
      ...
      try {
          if (user && user.uid !== 'demo-user-123') {
              const liveData = await getLiveStrategies(user.uid);
              setStrategies(liveData);
          } else {
  ...
  const filteredStrategies = strategies.filter(s => {
      const matchesSearch = s.clientName?.toLowerCase().includes(searchQuery.toLowerCase()) || s.theme.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesStatus = statusFilter === 'all' || s.status === statusFilter;
      const matchesClient = clientFilter === 'all' || s.clientId === clientFilter;
      return matchesSearch && matchesStatus && matchesClient;
  });

  const uniqueClients = Array.from(new Set(strategies.map(s => JSON.stringify({id: s.clientId, name: s.clientName})))).map((str) => JSON.parse(str as string));
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
        if (user && user.uid !== 'demo-user-123') {
            await deleteLiveStrategy(user.uid, strategyToDelete);
        } else {
            deleteLocalStrategy(strategyToDelete);
        }
        refreshStrategies();
        if (selectedStrategy?.id === strategyToDelete) setSelectedStrategy(null);
  ...
  const handleUpdateStatus = (strategy: Strategy, newStatus: Strategy['status'], e: React.MouseEvent) => {
    ...
    setActionMenuOpenId(null);
    success(`Strategy status updated to ${newStatus}`);
  };
  ...
  const handleGenerateSimulation = () => {
      ...
      setTimeout(() => {
          ...
          success("New strategy generated successfully!");
      }, 2000);
  };
  ...
  return (
    <div className="w-full space-y-6 animate-fade-in h-[calc(100vh-6rem)] flex flex-col relative">
        <ConfirmationModal
            ...
            message="This will permanently delete the strategy and its action plan. This cannot be undone."
        ...
        <div className="bg-white dark:bg-[#232323] p-4 rounded-lg border border-gray-200 dark:border-[#282828] shadow-sm shrink-0">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setIsGenerateModalOpen(true)} className="bg-emerald-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-emerald-700 transition-colors flex items-center gap-2 shadow-sm"><Sparkles size={16} /> Generate New Strategy</button>
            </div>
            <div className="flex flex-wrap gap-3 items-center">
                <div className="relative flex-1 min-w-[200px]"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} /><input type="text" placeholder="Search by client or theme..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-gray-50 dark:bg-[#1c1c1c] border border-gray-200 dark:border-[#383838] rounded-md text-sm outline-none focus:ring-1 focus:ring-emerald-500 text-gray-900 dark:text-[#ededed]" /></div>
                <div className="flex items-center gap-2"><Filter size={16} className="text-gray-400" /><select value={clientFilter} onChange={(e) => setClientFilter(e.target.value)} className="bg-gray-50 dark:bg-[#1c1c1c] border border-gray-200 dark:border-[#383838] rounded-md px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-emerald-500 text-gray-700 dark:text-[#ededed]"><option value="all">All Clients</option>{uniqueClients.map((c: any) => (<option key={c.id} value={c.id}>{c.name}</option>))}</select><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="bg-gray-50 dark:bg-[#1c1c1c] border border-gray-200 dark:border-[#383838] rounded-md px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-emerald-500 text-gray-700 dark:text-[#ededed]"><option value="all">All Statuses</option><option value="active">Active</option><option value="draft">Draft</option><option value="completed">Completed</option><option value="archived">Archived</option></select></div>
            </div>
        </div>
            ...
            <div className="overflow-auto custom-scrollbar flex-1">
                {filteredStrategies.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center p-12 text-center">
                        <div className="w-16 h-16 bg-gray-100 dark:bg-[#2a2a2a] rounded-full flex items-center justify-center mb-4 text-gray-400"><Layers size={32} /></div>
                        ...
                        <button onClick={() => setIsGenerateModalOpen(true)} className="mt-6 text-emerald-600 hover:text-emerald-700 font-medium text-sm flex items-center gap-2">Create First Strategy <ArrowRight size={16} /></button>
                    </div>
                ) : (
                    <table className="w-full text-left text-sm whitespace-nowrap">
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-[#2e2e2e]">
                            {filteredStrategies.map((strategy) => (
                                <tr key={strategy.id} className="hover:bg-gray-50 dark:hover:bg-[#2a2a2a] transition-colors cursor-pointer group" onClick={() => setSelectedStrategy(strategy)}>
                                    <td className="px-6 py-4"><div className="flex items-center gap-3"><img src={strategy.clientAvatar} alt="" className="w-8 h-8 rounded-full bg-gray-200 object-cover" /><span className="font-medium text-gray-900 dark:text-[#ededed]">{strategy.clientName}</span></div></td>
                                    <td className="px-6 py-4"><div className="flex flex-col"><span className="font-medium text-gray-900 dark:text-[#ededed]">{strategy.theme}</span><div className="flex items-center gap-1 text-xs text-gray-500 dark:text-[#8b9092] mt-0.5"><Calendar size={12} /> {strategy.month} {strategy.year}</div></div></td>
                                    <td className="px-6 py-4"><span className="bg-gray-100 dark:bg-[#2a2a2a] text-gray-600 dark:text-[#8b9092] px-2 py-1 rounded text-xs font-medium border border-gray-200 dark:border-[#383838]">{strategy.domain || 'General'}</span></td>
                                    <td className="px-6 py-4">{getStatusBadge(strategy.status)}</td>
                                    <td className="px-6 py-4"><div className="flex items-center gap-3"><div className="w-24 h-1.5 bg-gray-100 dark:bg-[#1c1c1c] rounded-full overflow-hidden"><div className={`h-full ${getPerformanceColor(strategy.performanceScore || 0)}`} style={{width: `${strategy.performanceScore || 0}%`}}></div></div><span className="text-xs font-medium text-gray-600 dark:text-[#8b9092]">{strategy.performanceScore || 0}%</span></div></td>
                                    <td className="px-6 py-4 text-right relative">
                                         <button onClick={(e) => { e.stopPropagation(); setActionMenuOpenId(actionMenuOpenId === strategy.id ? null : strategy.id); }} className="p-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-[#ededed] rounded-md hover:bg-gray-100 dark:hover:bg-[#333] transition-colors"><MoreVertical size={18} /></button>
                                         {actionMenuOpenId === strategy.id && (
                                             ...
                                             <div className="absolute right-8 top-8 w-56 bg-white dark:bg-[#1c1c1c] border border-gray-200 dark:border-[#383838] rounded-md shadow-lg z-50 animate-fade-in flex flex-col">
                                                 <button onClick={(e) => { e.stopPropagation(); info('Edit feature coming soon'); setActionMenuOpenId(null); }} className="w-full text-left px-4 py-2.5 text-xs hover:bg-gray-50 dark:hover:bg-[#2a2a2a] flex items-center gap-2 text-gray-700 dark:text-[#ededed] rounded-t-md"><Edit size={14} /> Edit Strategy</button>
                                                 <button onClick={(e) => { e.stopPropagation(); setSelectedStrategy(strategy); setActionMenuOpenId(null); }} className="w-full text-left px-4 py-2.5 text-xs hover:bg-gray-50 dark:hover:bg-[#2a2a2a] flex items-center gap-2 text-gray-700 dark:text-[#ededed]"><Eye size={14} /> View Details</button>
                                                 <button onClick={(e) => handleDuplicate(strategy, e)} className="w-full text-left px-4 py-2.5 text-xs hover:bg-gray-50 dark:hover:bg-[#2a2a2a] flex items-center gap-2 text-gray-700 dark:text-[#ededed]"><Copy size={14} /> Duplicate</button>
                                                 <div className="relative group">
                                                    <button className="w-full text-left px-4 py-2.5 text-xs hover:bg-gray-50 dark:hover:bg-[#2a2a2a] flex items-center justify-between text-gray-700 dark:text-[#ededed]"><div className="flex items-center gap-2"><RefreshCw size={14} /> Change Status</div><ChevronRight size={14} className="text-gray-400" /></button>
                                                    <div className="hidden group-hover:block absolute right-full top-0 w-32 bg-white dark:bg-[#1c1c1c] border border-gray-200 dark:border-[#383838] rounded-md shadow-lg mr-1 z-50">
                                                         {(['draft', 'active', 'completed', 'archived'] as const).map((status) => (<button key={status} onClick={(e) => handleUpdateStatus(strategy, status, e)} className={`w-full text-left px-4 py-2.5 text-xs hover:bg-gray-50 dark:hover:bg-[#2a2a2a] capitalize block first:rounded-t-md last:rounded-b-md ${strategy.status === status ? 'font-semibold text-emerald-600' : 'text-gray-700 dark:text-[#ededed]'}`}>{status}</button>))}
                                                    </div>
                                                 </div>
                                                 <button onClick={(e) => { e.stopPropagation(); success(`Report sent to ${strategy.clientName}`); setActionMenuOpenId(null); }} className="w-full text-left px-4 py-2.5 text-xs hover:bg-gray-50 dark:hover:bg-[#2a2a2a] flex items-center gap-2 text-gray-700 dark:text-[#ededed]"><Mail size={14} /> Email Report</button>
                                                 <div className="border-t border-gray-100 dark:border-[#383838] my-1"></div>
                                                 <button onClick={(e) => handleDeleteClick(strategy.id, e)} className="w-full text-left px-4 py-2.5 text-xs hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 flex items-center gap-2 rounded-b-md"><Trash2 size={14} /> Delete</button>
                                             </div>
                                             </>
                                         )}
                                    </td>
